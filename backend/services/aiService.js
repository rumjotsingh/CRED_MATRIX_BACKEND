...
const extractKeywords = async (text) => {
  const commonSkills = [
    ...
    // Frontend
    'React', 'Angular', 'Vue', 'HTML', 'CSS', 'SASS', 'Bootstrap', 'Tailwind', 'jQuery', 'Redux', 'Next.js',
    
    // Backend
    'Node.js', 'Express', 'Django', 'Flask', 'Spring', 'Laravel', 'ASP.NET', 'FastAPI', 'NestJS',
    
    // Databases
    'MongoDB', 'SQL', 'PostgreSQL', 'MySQL', 'Redis', 'Cassandra', 'Oracle', 'SQLite', 'DynamoDB',
    ...
    // Data Science & AI
    'Machine Learning', 'Deep Learning', 'AI', 'Data Science', 'TensorFlow', 'PyTorch', 'Scikit-learn',
    'Pandas', 'NumPy', 'Keras', 'NLP', 'Computer Vision', 'Data Analysis', 'Statistics',
    
    // Tools & Others
    'Git', 'GitHub', 'GitLab', 'REST API', 'GraphQL', 'Microservices', 'Agile', 'Scrum',
    'Testing', 'Jest', 'Selenium', 'Webpack', 'Linux', 'Bash', 'PowerShell'
  ];
...
const categorizSkill = (skill) => {
  const categories = {
    technical: ['programming', 'web development', 'data science', 'machine learning', 
                'cloud computing', 'database', 'frontend', 'backend', 'mobile development',
...
const predictNSQFFromKeywords = (text, type) => {
  ...
  const levelKeywords = {
    ...
    6: ['bachelor', 'degree', 'graduate', 'advanced diploma', 'professional', 'undergraduate'],
    7: ['postgraduate', 'masters', 'specialized', 'expert', 'advanced professional'],
    8: ['masters degree', 'postgraduate diploma', 'advanced professional', 'mba'],
    9: ['doctoral', 'phd', 'research', 'highly specialized', 'doctorate'],
    10: ['doctorate', 'phd', 'research doctorate', 'highest qualification', 'post-doctoral']
  };
...
const parseCareerRecommendations = (text, skills) => {
  ...
  careerPaths.forEach(career => {
    ...
    career.requiredSkills.forEach(reqSkill => {
      if (Array.from(skillSet).some(skill => skill.includes(reqSkill) || reqSkill.includes(skill))) {
        matchCount++;
      }
    });

    const matchScore = Math.round((matchCount / career.requiredSkills.length) * 100);
...
export const analyzeSkillGap = async (currentSkills, targetRole) => {
  try {
    ...
    if (requiredSkills.length === 0) {
      return {
        missingSkills: [],
        recommendations: [`Role "${targetRole}" not found. Available roles: ${Object.keys(getAllRoleSkills()).join(', ')}`],
        matchPercentage: 0,
        requiredSkills: [],
    ...
    const skillMatches = await analyzeSkillMatches(currentSkillNames, requiredSkills);
...
const extractClassificationConfidence = (result, label) => {
  ...
  if (Array.isArray(result) && result[0]) {
    ...
    if (Array.isArray(scores)) {
      const yesScore = scores.find(s => s.label && s.label.toLowerCase().includes(label));
 ...
 */
const generateSkillGapRecommendations = async (missingSkills, targetRole, matchPercentage) => {
  const recommendations = [];
  ...
  if (missingSkills.length > 0) {
    ...
    topMissingSkills.forEach(skill => {
      recommendations.push(`Learn ${skill} through online courses, tutorials, or certifications`);
    });
    
    if (missingSkills.length > 5) {
      recommendations.push(`Also consider learning: ${missingSkills.slice(5).join(', ')}`);
    }
  } else {
    recommendations.push(`Great! You have all the required skills for ${targetRole}`);
    recommendations.push('Consider building projects to strengthen your portfolio');
    recommendations.push('Stay updated with latest trends and technologies in your field');
  }
...
const getAllRoleSkills = () => {
  return {
    'Software Developer': ['JavaScript', 'Python', 'Java', 'Git', 'SQL', 'Data Structures', 'Algorithms', 'OOP', 'Testing', 'Debugging'],
    'Software Engineer': ['JavaScript', 'Python', 'Java', 'Git', 'SQL', 'Data Structures', 'Algorithms', 'OOP', 'Testing', 'Debugging'],
    'Full Stack Developer': ['JavaScript', 'React', 'Node.js', 'Express', 'MongoDB', 'SQL', 'HTML', 'CSS', 'Git', 'REST API'],
    'Data Scientist': ['Python', 'Machine Learning', 'Statistics', 'SQL', 'Data Visualization', 'Pandas', 'NumPy', 'Scikit-learn', 'R', 'Deep Learning'],
    'DevOps Engineer': ['Docker', 'Kubernetes', 'CI/CD', 'AWS', 'Linux', 'Git', 'Jenkins', 'Terraform', 'Monitoring', 'Scripting'],
    'Frontend Developer': ['JavaScript', 'React', 'HTML', 'CSS', 'TypeScript', 'Webpack', 'Git', 'Responsive Design', 'UI/UX', 'Testing'],
    'Backend Developer': ['Node.js', 'Python', 'Java', 'SQL', 'MongoDB', 'REST API', 'Microservices', 'Git', 'Authentication', 'Security'],
    'Mobile Developer': ['React Native', 'Flutter', 'iOS', 'Android', 'Mobile UI/UX', 'API Integration', 'Git', 'App Store', 'Testing'],
    'Cloud Architect': ['AWS', 'Azure', 'Cloud Security', 'Networking', 'Infrastructure as Code', 'Kubernetes', 'Docker', 'Serverless', 'Cost Optimization'],
    'Machine Learning Engineer': ['Python', 'TensorFlow', 'PyTorch', 'Machine Learning', 'Deep Learning', 'Data Processing', 'MLOps', 'Statistics', 'Model Deployment'],
    'Web Developer': ['HTML', 'CSS', 'JavaScript', 'React', 'Node.js', 'Git', 'Responsive Design', 'REST API', 'Database', 'Testing'],
    'UI/UX Designer': ['Figma', 'Adobe XD', 'Sketch', 'Prototyping', 'User Research', 'Wireframing', 'Design Systems', 'HTML', 'CSS'],
    'Database Administrator': ['SQL', 'MongoDB', 'PostgreSQL', 'MySQL', 'Database Design', 'Performance Tuning', 'Backup', 'Security', 'Replication'],
    'QA Engineer': ['Testing', 'Selenium', 'Jest', 'Test Automation', 'Bug Tracking', 'API Testing', 'Performance Testing', 'Git', 'CI/CD'],
    'Product Manager': ['Product Strategy', 'Agile', 'Scrum', 'User Stories', 'Roadmapping', 'Analytics', 'Communication', 'Stakeholder Management'],
    'Project Manager': ['Project Planning', 'Agile', 'Scrum', 'Risk Management', 'Budgeting', 'Communication', 'Leadership', 'MS Project', 'JIRA'],
    'Business Analyst': ['Requirements Analysis', 'SQL', 'Data Analysis', 'Documentation', 'Stakeholder Management', 'Process Modeling', 'Excel', 'Communication'],
    'Data Analyst': ['SQL', 'Excel', 'Python', 'Data Visualization', 'Tableau', 'Power BI', 'Statistics', 'Data Cleaning', 'Reporting'],
    'Cybersecurity Specialist': ['Network Security', 'Penetration Testing', 'Cryptography', 'Security Auditing', 'Firewall', 'SIEM', 'Incident Response', 'Compliance'],
    'Network Engineer': ['Networking', 'TCP/IP', 'Routing', 'Switching', 'Firewall', 'VPN', 'Network Security', 'Troubleshooting', 'Cisco', 'Linux'],
    'System Administrator': ['Linux', 'Windows Server', 'Networking', 'Scripting', 'Virtualization', 'Backup', 'Security', 'Monitoring', 'Troubleshooting'],
...
const analyzeSkillGapFallback = (currentSkills, targetRole) => {
  ...
  if (requiredSkills.length === 0) {
    return {
      missingSkills: [],
      recommendations: [`Role "${targetRole}" not found. Available roles: ${Object.keys(roleSkills).join(', ')}`],
      matchPercentage: 0,
      requiredSkills: [],
      currentSkills: currentSkills,
  ...
  if (missingSkills.length > 0) {
    ...
    recommendations.push(`Great! You have all the required skills for ${normalizedRole}`);
    recommendations.push('Consider building projects to strengthen your portfolio');
    recommendations.push('Stay updated with latest trends and technologies in your field');
  }
